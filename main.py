...
if isinstance(modelsListName, str):
    ...
    for key in dict_prediction[modelsListName].keys():
        ...
        dict_prediction[modelsListName][key] = preds
        dict_error[modelsListName][key] = { key : list() for key in [revisionMethodName]} if isinstance(revisionMethodName, str) else { key : list() for key in revisionMethodName}
        y_train = train
        y_true = test
...
y_pred = preds[-28:]
x =  jsonResult(ht, modelsListName, revisionMethodName, dict_prediction, y_true, dict_error)
